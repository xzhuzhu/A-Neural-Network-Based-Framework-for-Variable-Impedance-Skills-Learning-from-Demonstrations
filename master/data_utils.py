...
class LASA:
    ...
    def load_data(self, data_name):
        dataset_path = 'lasa_handwriting_dataset'
        downsample_rate = 4
        start_cut = 10
        end_cut = 5
        tol_cutting = 1.
        ...
        num_demos = int(dataset.shape[1]) #round(int(dataset.shape[1])/2)#
...
class GLASA:
    ...
    def plot_data(self):
        fig = plt.figure()
        ax = fig.gca()
        x = (self.x) / self.scaling

        ax.scatter(x[:, 0], x[:, 1])
        plt.plot(0, 0, 'o', linewidth=10, markersize=4, markeredgecolor='black')
    ...
    def load_data(self, data_name):
        ...
        num_demos = round(int(dataset.shape[1])/3)
        d = dataset[0][0]['pos'][0][0].shape[0]
        ...
        xd1 = []
        for i in range(num_demos):
            x1.append(dataset[0][i]['pos'][0][0])
            xd1.append(dataset[0][i]['vel'][0][0])
        ...
        #
        for i in range(num_demos):
            demo = x1[i] #dataset[0, i]['pos'][0][0]#[:, ::downsample_rate]
            num_pts = demo.shape[1]
        ...
        x = x.T
        xd = xd.T
        # xd = dataset[0, 0]['vel'][0][0]
        goal = x[-1].reshape(1, -1)
        scaling = scaling.T
        translation = translation.T

        return [x.astype(np.float32), xd.astype(np.float32), idx, 0.01, goal, scaling, translation]

# GLASA(dato9a_name='S').plot_data()