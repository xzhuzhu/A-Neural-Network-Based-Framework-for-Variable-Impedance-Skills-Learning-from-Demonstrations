...
def sim_euler(dynamics, state_0, t_final, dt, t_init=None):
    ...
    return t, y,state_y[:,1:-1]


def generate_trajectories(
        model, x_init, time_dependent=False, order=2,via_point=None,
    ...
    if method == 'ivp':
        ...
        t, y,zz = sim_euler(dynamics, x_init, t_final, t_step, t_init=t_init)
        x_data = torch.from_numpy(y.T).float()
        z_data = torch.from_numpy(zz.T).float()
    else:
